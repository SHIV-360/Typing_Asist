...
def show_sentence():
    ...
    if not selected_difficulty or not selected_time:
        sentence_label.config(text="Please select difficulty & time first then type start and press enter.", fg="red")
        return
...
def stop_timer():
    ...
    display_typing_metrics_popup()  # Call the updated metrics popup window
        # Append data from a.csv to b.csv, ensuring no extra blank line
    with open(a_file, 'r') as a, open(b_file, 'a') as b:
        # Check if b.csv is empty, if so, don't add a newline
        if b.tell() > 0:
            b.write("\n")  # Add a newline only if b.csv is not empty
        b.write(a.read())  # Append the content of a.csv to b.csv
...
def display_typing_metrics_popup():
    ...
    result_text = f"Total Letters: {total_letters}\n"
    result_text += f"Correct Letters: {correct_letters}\n"
    result_text += f"Correct Letters Per Second: {correct_letters_per_second:.2f}"
...
difficulty_label = tk.Label(window, text="Select Difficulty:", font=("Comic Sans MS", 13, "bold"), fg="white", bg="#222222")
...
# Time selection
time_label = tk.Label(window, text="Select Time:", font=("Comic Sans MS", 13, "bold"), fg="white", bg="#222222")
time_label.pack(pady=5)

time_frame = tk.Frame(window, bg="#222222")
time_frame.pack()
...
sentence_label = tk.Label(window, text="", wraplength=600, font=("Comic Sans MS", 14, "bold"), justify="center", bg="#333333", fg="white")
sentence_label.pack(pady=20, fill="both")

user_input_entry = tk.Entry(window, font=("Comic Sans MS", 12), width=60, justify="center")
user_input_entry.pack(pady=5)
user_input_entry.bind('<Return>', lambda event: show_sentence())  # Capture input on Enter

timer_label = tk.Label(window, text="", font=("Comic Sans MS", 14, "bold"), fg="white", bg="#222222")
timer_label.pack(pady=5)

metrics_label = tk.Label(window, text="", font=("Comic Sans MS", 14, "bold"), fg="white", bg="#222222")
metrics_label.pack(pady=20)

tk.Button(window, text="Reset", font=("Comic Sans MS", 12, "bold"), bg="red", fg="white", command=reset_app).pack(pady=10)
